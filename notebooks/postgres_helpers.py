...
def get_table_info(pg_conn, table_name):
    ...
    result : Pandas DataFrame
            returns results as pandas dataframe
    """
    query = f"""SELECT column_name, udt_name, data_type, character_maximum_length
    FROM information_schema.columns 
    WHERE table_name = '{table_name}';"""
    result = pd.read_sql(pg_conn, query)
    return result


if __name__ == "__main__":
    pg_conn = create_connection(verbose=True)
    try:
        info = get_table_info(pg_conn, 'titanic')
        print(info)
